...
public final class LoggerFactory {
    ...
    private final static void bind() {
        try {
            ...
            List<SLF4JServiceProvider> clearServiceProviders = clearProvidersList(providersList);
            reportMultipleBindingAmbiguity(clearServiceProviders);
            if (clearServiceProviders != null && !clearServiceProviders.isEmpty()) {
                PROVIDER = clearServiceProviders.get(0);
                // SLF4JServiceProvider.initialize() is intended to be called here and nowhere else.
                PROVIDER.initialize();
                INITIALIZATION_STATE = SUCCESSFUL_INITIALIZATION;
                reportActualBinding(clearServiceProviders);
            } else {
